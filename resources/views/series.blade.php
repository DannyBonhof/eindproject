...
@foreach ($genresToShow as $label => $genre)
    ...
    <div class="scroll-wrapper">
      <button class="scroll-btn scroll-left" onclick="scrollShows(-200, '{{ Str::slug(is_array($genre) ? implode('-', $genre) : $genre) }}')" aria-label="Scroll naar links">&#8592;</button>
      <div class="scroll-container" id="scrollContainer-{{ Str::slug(is_array($genre) ? implode('-', $genre) : $genre) }}">
        <?php
      ...
      <button class="scroll-btn scroll-right" onclick="scrollShows(200, '{{ Str::slug(is_array($genre) ? implode('-', $genre) : $genre) }}')" aria-label="Scroll naar rechts">&#8594;</button>
    </div>
@endforeach
